...
import {privateKeyToAccount} from "iotex-antenna/lib/crypto/crypto";
...
const EXPLORER = {
  "name": "iotexscan",
  "account": "https://iotexscan.io/address/{x}",
  "transaction": "https://iotexscan.io/action/{x}",
  "block": "https://iotexscan.io/block/{x}"
};

export default class IOTX extends Plugin {

  constructor() {
    super(Blockchains.IOTX, PluginTypes.BLOCKCHAIN_SUPPORT)
  }
  ...
  returnableAccount(account) {
    return {address: account.publicKey, blockchain: Blockchains.IOTX}
  }
  ...
  getEndorsedNetwork() {
    return new Network('IOTX Mainnet', 'http', 'api.iotex.one', 8080, Blockchains.IOTX, '1')
  }

  isEndorsedNetwork(network) {
    const endorsedNetwork = this.getEndorsedNetwork();
    return network.blockchain === Blockchains.IOTX && network.chainId === endorsedNetwork.chainId;
  }
  ...
  privateToPublic(privateKey) {
    return privateKeyToAccount(privateKey).publicKey;
  }
  ...
  defaultToken() {
    return new Token(Blockchains.IOTX, 'iotx', 'IOTX', 'IOTX', this.defaultDecimals(), '1')
  }
